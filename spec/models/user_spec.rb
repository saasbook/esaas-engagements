...
describe User do
    ...
    describe 'User associations' do
        ...
        it 'can produce participating engagements by user type' do
            @user1 = User.create!(:name => 'coach', :email => 'user1@email.com', :user_type => 'client')
            @user2 = User.create!(:name => 'client', :email => 'user2@email.com', :user_type => 'coach')
            @user3 = User.create!(:name => 'student', :email => 'user3@email.com', :user_type => 'student')
            @user4 = User.create!(:name => 'random', :email => 'user4@email.com')

            @org1 = @user1.client_orgs.create!(:name => 'org1')
            @app1 = @org1.apps.create!(:name => 'app1', :description => 'wow', :status => :pending )
            @eng1 = @app1.engagements.create!(:coach_id => 1, :team_number => '1', :start_date => DateTime.new(2017,2,3,4,5,6))

            @eng1.update_attributes!(coach: @user2)
            @eng1.developers << @user3

            expect(@user1.participating_engagements).to eq [@eng1]
            expect(@user2.participating_engagements).to eq [@eng1]
            expect(@user3.participating_engagements).to eq [@eng1]
